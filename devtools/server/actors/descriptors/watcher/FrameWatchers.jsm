...
// While DevToolsFrameParent is instanciated per WindowGlobal.
// We record a Map of all available Watchers, indexed by their Connection's prefix string.
// This later help notifying about the target actor, created from the content processes
...
async function registerWatcher(watcher, watchedBrowsingContextID) {
  ...
  for (const browsingContext of browsingContexts) {
    ...
    await browsingContext.currentWindowGlobal
      ...
      .sendQuery("instantiate-already-available", {
        prefix,
...
function shouldNotifyWindowGlobal(windowGlobal, watchedBrowsingContextID) {
  ...
  // Ignore globals running in the parent process as JSWindowActor API won't work

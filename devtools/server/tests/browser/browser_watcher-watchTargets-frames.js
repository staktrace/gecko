...
const TEST_DOC_URL2 =
  MAIN_DOMAIN.replace("test1.example.org", "test3.example.net") +
  "doc_iframe2.html";
...
async function assertTabIFrames(watcher, targets) {
  ...
  if (SpecialPowers.useRemoteSubframes) {
    ...
    is(sameTarget, existingIframeTarget, "getBrowsingContextTarget returns the same target");
    const tabTarget = targets.find(f => f.url == TEST_DOC_URL);
    ok(tabTarget, "Found the tab target");
    const sameTabTarget = await watcher.getParentBrowsingContextTarget(existingIframeId);
    is(tabTarget, sameTabTarget, "getParentBrosingContextTarget returned the tab target");
  } else {
    ok(

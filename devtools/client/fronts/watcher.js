...
class WatcherFront extends FrontClassWithSpec(watcherSpec) {
  ...
  async getBrowsingContextTarget(id) {
    ...
    // The parent front of the WatcherActor happens to be the Descritpor Actor.
    ...
    for (const front of this.parentFront.poolChildren()) {
      // And also look into LocalTabTargets also managed by the root front
      if (front.browsingContextID == id) {
        return front;
      }
    }
...
exports.WatcherFront = WatcherFront;

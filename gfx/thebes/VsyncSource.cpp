...
void VsyncSource::Display::NotifyVsync(TimeStamp aVsyncTimestamp) {
  ...
  bool dispatchToMainThread = (mVsyncId == mLastMainThreadProcessedVsyncId);
  ...
  if (dispatchToMainThread) {
    NS_DispatchToMainThread(NewRunnableMethod<VsyncEvent>(
        ...
        &VsyncSource::VsyncDisplay::NotifyGenericObservers, event);
  }
}
...
void VsyncSource::Display::MoveListenersToNewSource(
  ...
  aNewDisplay.mGenericObservers.AppendElements(
      std::move(mGenericObservers));

...
APZCTreeManager::HitTestResult APZCTreeManager::GetAPZCAtPointWR(
  ...
  for (const wr::WrHitResult& result : results) {
    ...
    RefPtr<HitTestingTreeNode> node = GetTargetNode(guid,
        &GuidComparatorIgnoringPresShell);
    if (!node) {
      // Not sure exactly what to do here, we might want to `continue`.
      // For now let's do the thing that is most consistent with the old
      // behaviour.
